...
if [ ! -d "`eval echo ${DESTINATION//>}`" ]; then
    ...
    mkdir Helm
    mkdir Manifest
    curl -L -o master.zip https://github.com/martinspaniol/Homelab/archive/refs/heads/main.zip
    unzip master.zip -d ~/Homelab
    cp -r ~/Homelab/Homelab-main/Kubernetes/Traefik/* ~/
    rm master.zip
    rm -r ~/Homelab
